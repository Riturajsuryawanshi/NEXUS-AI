...
export function MagneticText({ text, hoverText, className, textClassName }: MagneticTextProps) {
  ...
  return (
    ...
    >
      ...
      <span className={cn("text-5xl font-bold tracking-tighter text-foreground", textClassName)}>{text}</span>

      <div
        ref={circleRef}
        className="absolute top-0 left-0 pointer-events-none rounded-full bg-slate-900 overflow-hidden"
        style={{
      ...
      >
        ...
        >
          <span className={cn("text-5xl font-bold tracking-tighter text-white whitespace-nowrap", textClassName)}>
            {hoverText || text}
          </span>
        </div>
      </div>
